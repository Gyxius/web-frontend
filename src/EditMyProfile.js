...
function EditMyProfile({ userName, joinedEvents = [], onBack, onSignOut, startEditing = false, onAccessAdminPanel }) {
  const theme = {
    bg: "#F7F7F5",
    card: "#FFFFFF",
    text: "#1F2937",
  ...
  */
  ...
  const canonicalizeCountry = (val) => {
    ...
    const match = FULL_COUNTRIES.find(c => c.toLowerCase() === trimmed.toLowerCase());
